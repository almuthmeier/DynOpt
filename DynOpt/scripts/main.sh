...
pred1="tcn"
#red2="tcn"
algnameaddition1="_auto_predDEV"								# TCN
...
for d in "${dims[@]}"
do
	sbatch --mem=35G --job-name="d$d-at_DEV" --output="slurm_d$d-at_DEV.%j.out" --error="slurm_d$d-at_DEV.%j.err" subscript2.job "$pred1" "$algnameaddition1" "$useuncs1" "$epuncfactor1" "$reinimode" "$zfactors" "$d" &
done
