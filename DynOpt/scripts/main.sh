...
for d in "${dims[@]}"
do
	sbatch --mem=24G --job-name="d$d-k_RND" --output="slurm_d$d-k_predRND.%j.out" --error="slurm_d$d-k_predRND.%j.err" subscript2.job "$pred1" "$algnameaddition1" "$useuncs1" "$epuncfactor" "$reinimode1" "$zfactors" "$d" &
	sbatch --mem=24G --job-name="d$d-k_DEV" --output="slurm_d$d-k_predDEV.%j.out" --error="slurm_d$d-k_predDEV.%j.err" subscript2.job "$pred2" "$algnameaddition2" "$useuncs2" "$epuncfactor" "$reinimode2" "$zfactors" "$d" &
done
