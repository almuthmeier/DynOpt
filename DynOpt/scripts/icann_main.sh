...
do
	...
	# SRR
	sbatch --mem=8G --job-name="d$d-kdev" --output="slurm_d$d-kdev.%j.out" --error="slurm_d$d-kdev.%j.err" icann_subscript.job "$pred1" "$algnameaddition1" "$useuncs1" "$reinimode1" "$zfactors" "$d" &
