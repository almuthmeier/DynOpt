...
def weightNormConvolution1d(x, num_filters, dilation_rate, filter_size=3, stride=[1],
    ...
    with tf.variable_scope(name):
        ...
        if init:
            ...
            W = tf.reshape(g, [1, 1, num_filters]) * \
                tf.nn.l2_normalize(V, [0, 1])
            ...
            x = tf.nn.bias_add(tf.nn.convolution(
                x, W, pad, stride, [dilation_rate]), b)
    ...
    with tf.variable_scope(name):
        ...
        if use_highway:
            ...
            b_h = tf.get_variable('b_h', shape=[out_channels], dtype=tf.float32,
                                  initializer=None, trainable=True)
            H = tf.nn.bias_add(tf.nn.convolution(
                input_layer, W_h, 'SAME'), b_h)
            ...
            b_t = tf.get_variable('b_t', shape=[out_channels], dtype=tf.float32,
                                  initializer=None, trainable=True)
            T = tf.nn.bias_add(tf.nn.convolution(
                input_layer, W_t, 'SAME'), b_t)
            T = tf.nn.sigmoid(T)
