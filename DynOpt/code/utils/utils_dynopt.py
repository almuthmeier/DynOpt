...
def compute_real_gens_of_chgs(periods_for_gens, act_n_chngs):
    '''
    @param periods_for_gens: 1d numpy array containing for each generation the 
    change period number it belongs to (as generated by generate_period_lengths())
    @param act_n_chngs: the real number of changes (not only those the EA detected)
    @return dictionary: for each change (even if the EA did not detect it) the 
    corresponding generations
    '''
    n_gens = len(periods_for_gens)
    real_gens_of_chgs = {0: [0]}

    if n_gens > 1:
        change_nr = 0
        for i in range(1, len(periods_for_gens)):
            if periods_for_gens[i - 1] == periods_for_gens[i]:
                real_gens_of_chgs[change_nr].append(i)
            else:
                change_nr += 1
                real_gens_of_chgs[change_nr] = [i]
    # +1, because number of changes is smaller than number of periods
    assert len(real_gens_of_chgs) == (act_n_chngs + 1)
    return real_gens_of_chgs


def generate_period_lengths(ea_np_rnd_generator, problem, max_n_gens,
                            len_change_period, n_changes, is_change_time_random,
                            pos_chng_type, fit_chng_type):
    '''
    ...
    '''
    # TODO "none_fit_ch" zu "non-fit-ch" umschreiben
    if (pos_chng_type == "none_pos_ch" and fit_chng_type == "none_fit_ch"
            and problem != "mpb"):
        # no change will occur
        periods_for_gens = np.zeros(max_n_gens, int)
        act_n_chngs = 0
    elif not is_change_time_random:
        # deterministic change time points (for MPB too)
        act_n_chngs = math.ceil(max_n_gens / len_change_period) - 1
        periods_for_gens = np.array(
            [gen // len_change_period for gen in range(max_n_gens)])
    else:
        # random change time points
        unsorted_periods_for_gens = ea_np_rnd_generator.randint(
            0, n_changes + 1, max_n_gens)
        periods_for_gens = np.sort(unsorted_periods_for_gens)
        # it must exist one period more than change numbers
        assert len(np.unique(periods_for_gens)) == n_changes + \
            1, "length unequal"
        act_n_chngs = n_changes
    return periods_for_gens, act_n_chngs
